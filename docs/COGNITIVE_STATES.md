...
- **MemoryRank**: 
  - `local_weight_boost=3.0` (개념적) → 로컬 연결 강화로 패턴 고착
  - 실제로는 edges의 weight가 높으면 특정 패턴이 강화됨
- **PFC**: 
  - `decision_temperature=5.0` (높음) → `P(i) = exp(β×U_i)`에서 β가 높아서 효용 차이 강조
  - 최적 선택(높은 U)만 강하게 선택 → 익숙한 패턴에서 벗어나지 않음
- **Thalamus**: 
  - `gate_threshold=0.0` (최소) → 모든 미세 자극 통과 (감각 과부하)
  - `max_channels=1` → 단일 채널 집중 (다른 자극 차단)
...
| ADHD | 높음 (3~4개) | 80% | 산만함, 계속 전환 |
| ASD | 낮음 (1~2개) | 20% | 패턴 고착, 새 옵션 거부 |

**선택 분산 해석**:
- **높은 분산**: 다양한 선택 → 탐색 강화
- **낮은 분산**: 선택 공간 수렴 → 착취 강화
